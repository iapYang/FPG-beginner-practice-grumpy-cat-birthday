...
function deleteUselessImage(targetDir, config){
	...
	console.log('Delteted ' + num + ' files');
}
// console.log(fileUtil.listDir(config.targetDir, false));

module.exports = {deleteImgs: function(){
	var targetDir = config.targetDir?config.targetDir:__dirname.substring(0,__dirname.length-9)+'toClient\\creative';
	// console.log(targetDir);
	if(config.batch){
		var targetDirs = fileUtil.listDir(targetDir, false);
		...
		deleteUselessImage(targetDir, config)
	}
}}
